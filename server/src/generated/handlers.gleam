...
fn verify_auth(state: State) -> Result(types.User, Nil) {
  case state.bearer_token {
    None -> Error(Nil)
    Some(token) -> auth.verify_token(state.conn, token) |> result.map_error(fn(_) { Nil })
  }
}
...
pub fn get_featured_items(
  ...
  let items = case req.body {
    ...
    Some(lat_lng) ->
      featured_items.get_featured_items(state.conn, lat_lng)
  }
...
pub fn create_item(
  ...
  case verify_auth(state) {
    Error(_) -> response_types.CreateItemResponseCreated(types.CreateItemResponse(id: 0))
    Ok(user) -> {
      case body.city == "" || body.postal_code == "" {
        ...
        False -> {
          case sql.create_item(
            ...
            Ok(returned) -> case list.first(returned.rows) {
              Ok(row) ->
                response_types.CreateItemResponseCreated(types.CreateItemResponse(id: row.id))
              Error(_) -> {
            ...
            Error(e) -> {
              io.println("[handlers] DB error creating item: " <> string.inspect(e))
              response_types.CreateItemResponseCreated(types.CreateItemResponse(id: 0))
            }
          }
        }
  ...
  case result {
    ...
    Ok(returned) -> {
      case list.first(returned.rows) {
        ...
        Ok(row) -> {
          let images_result = sql.get_item_images_for_item(state.conn, req.item_id)
                ...
                case item_row.author_id == user.id {
                  ...
                  True -> {
                    case sql.update_item(
                      ...
                      Error(_) -> response_types.UpdateItemResponseNotFound
                      Ok(updated) -> case list.first(updated.rows) {
                        Error(_) -> response_types.UpdateItemResponseNotFound
                        Ok(row) ->
...
pub fn upload_item_image(
  ...
  case verify_auth(state) {
    Error(_) -> response_types.UploadItemImageResponseCreated(
                ...
                Ok(_) -> {
                  ...
                  case sql.insert_item_image(
                    ...
                    Ok(returned) -> case list.first(returned.rows) {
                      ...
                      Ok(_row) -> {
                        io.println("[handlers] Created image id=" <> uuid_string)
                        response_types.UploadItemImageResponseCreated(
              ...
              True -> {
                list.each(req.body.delete, fn(uuid_string) {
                  case uuid.from_string(uuid_string) {
                    Error(_) ->
                      io.println("[handlers] Invalid UUID in delete: " <> uuid_string)
                    Ok(parsed_uuid) -> {
                      let _ = delete_image_file(state.conn, req.item_id, parsed_uuid, uuid_string)
                      Nil
                    }
                  }
                })
                ...
                list.each(req.body.reorder, fn(entry: types.ReorderEntry) {
                  case uuid.from_string(entry.uuid) {
                    Error(_) ->
                      io.println("[handlers] Invalid UUID in reorder: " <> entry.uuid)
                    Ok(parsed_uuid) -> {
                      let _ = sql.update_item_image_sort_order(
                        state.conn,
                        parsed_uuid,
                        req.item_id,
                        entry.sort_order,
...
fn delete_image_file(
  ...
  case sql.delete_item_image(conn, image_uuid, item_id) {
    Error(e) -> io.println("[handlers] DB error deleting image: " <> string.inspect(e))
    Ok(result) -> {
      case result.rows {
        [] -> Nil
        [row, ..] -> {
