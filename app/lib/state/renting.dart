...
final rentRequestProvider =
    FutureProvider.autoDispose.family<RentRequest?, int>((ref, requestId) async {
  try {
...
final messagesProvider =
    FutureProvider.autoDispose.family<List<Message>, int>((ref, requestId) async {
  final result = await AppConfig.apiClient.getMessages(requestId);
  return result ?? [];
});

final offersProvider =
    FutureProvider.autoDispose.family<List<RentOffer>, int>((ref, requestId) async {
...
Future<RentOffer?> createOffer(int requestId, DateTime startDate, DateTime endDate) async {
  try {
