...
fn run() -> Result(Nil, String) {
  ...
  use _ <- result.try(
    migration.run_all()
    |> result.map_error(migration.message),
  )

  use _ <- result.try(clear_all(conn))

  use dev_user <- result.try(create_user_with_profile(
    conn,
    "dev@example.com",
    "dev",
    "Dev",
  ...
  use lisa <- result.try(create_user_with_profile(
    conn,
    "lisa@example.com",
    "lisa",
    "Lisa",
  ...
  use luca <- result.try(create_user_with_profile(
    conn,
    "luca@example.com",
    "luca",
    "Luca",
  ...
  use timon <- result.try(create_user_with_profile(
    conn,
    "timon@example.com",
    "timon",
    "Timon",
  ...
  use inserat1_id <- result.try(insert_item(
    conn,
    "Inserat 1",
    "Ganz tolles Inserat",
    dev_user.id,
  ...
  use internat_id <- result.try(insert_item(
    conn,
    "Internat",
    "Ganz tolles Internat",
    dev_user.id,
    4.9,
    11.582,
    48.1351,
  ...
  use inserat2_id <- result.try(insert_item(
    conn,
    "Inserat 2",
    "Papput",
    dev_user.id,
  ...
  use auto_id <- result.try(insert_item(
    conn,
    "Auto",
    "Kann fahren",
    lisa.id,
  ...
  use spezi_id <- result.try(insert_item(
    conn,
    "Spezi",
    "Bitte voll zurueck",
    timon.id,
  ...
  use fahrrad_id <- result.try(insert_item(
    conn,
    "Fahrrad",
    "Trekkingrad, 28 Zoll, 21 Gänge",
    lisa.id,
    4.1,
    6.9603,
    50.9375,
  ...
  use bohrmaschine_id <- result.try(insert_item(
    conn,
    "Bohrmaschine",
    "Professionelle Bohrmaschine für Heimwerker",
    luca.id,
    4.5,
    9.18,
    48.78,
  ...
  use zelt_id <- result.try(insert_item(
    conn,
    "Zelt",
    "Geräumiges 4-Personen Zelt für Campingausflüge",
    luca.id,
    4.2,
    9.18,
    48.78,
  ...
  use _ <- result.try(seed_image(
    conn,
    spezi_id,
    0,
    "seeding/images/paulaner-spezi.jpg",
  ))
  use _ <- result.try(seed_image(
    conn,
    spezi_id,
    1,
    "seeding/images/wallhaven_4576l3.jpg",
  ))
  use _ <- result.try(seed_image(
    conn,
    spezi_id,
    2,
    "seeding/images/logo_v1.jpeg",
  ))
  use _ <- result.try(seed_image(
    conn,
    spezi_id,
    3,
    "seeding/images/wallhaven_weq5jq.jpg",
  ))
  ...
  use _ <- result.try(seed_image(
    ...
    auto_id,
    1,
    "seeding/images/IMG_0642.png",
  ))
  use _ <- result.try(seed_image(
    conn,
    auto_id,
    2,
    "seeding/images/wallhaven_477mrv.jpg",
  ))
  use _ <- result.try(seed_image(
    conn,
    auto_id,
    3,
    "seeding/images/wallhaven_nkmxk6.jpg",
  ))
  ...
  use _ <- result.try(seed_image(
    ...
    inserat1_id,
    0,
    "seeding/images/wallhaven_4gqvxd.jpg",
  ))
  use _ <- result.try(seed_image(
    conn,
    inserat1_id,
  ...
  use _ <- result.try(seed_image(
    ...
    internat_id,
    0,
    "seeding/images/wallhaven_e7651w.jpg",
  ))
  ...
  use _ <- result.try(seed_image(
    ...
    inserat2_id,
    0,
    "seeding/images/wallhaven_mdgzx9.jpg",
  ))

  use _ <- result.try(seed_image(conn, fahrrad_id, 0, "seeding/images/bike.png"))
  ...
  use _ <- result.try(seed_rent_data(
    conn,
    dev_user,
    lisa,
    timon,
...
fn seed_rent_data(
  ...
  // ──────────────────────────────────────────────
  // A: Lisa → Spezi (Timon) — unrelated to Dev, OPEN, 1 offer
  // ──────────────────────────────────────────────
  use returned <- result.try(
    sql.create_rent_request(conn, spezi_id, lisa.id)
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_a.id,
      lisa.id,
      "Hey, kann ich die Spezi fürs Wochenende ausleihen?",
    )
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_a.id,
      lisa.id,
      "Von Freitag bis Sonntag, 20.–22. Juni",
    )
  ...
  use _ <- result.try(
    sql.mark_rent_request_read(conn, req_a.id, lisa.id)
    |> result.map_error(fn(_) { "A: mark_read lisa" }),
  )
  use _ <- result.try(
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_b.id,
      dev_user.id,
      "Moin, kann ich das Auto für den Umzug am 1. Juli leihen?",
    )
    |> result.map_error(fn(_) { "B: msg 1" }),
  )
  use _ <- result.try(
    sql.create_message(conn, req_b.id, lisa.id, "Klar, ein Tag reicht?")
  ...
  use returned <- result.try(
    sql.create_offer(conn, req_b.id, lisa.id, ob_start, ob_end)
  ...
  use _ <- result.try(
    sql.mark_rent_request_read(conn, req_b.id, lisa.id)
    |> result.map_error(fn(_) { "B: mark_read lisa" }),
  )
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_c.id,
      dev_user.id,
      "Hey, kann ich die Spezi auch mal ausleihen?",
    )
  ...
  use oc1 <- result.try(
    returned.rows
    |> list.first
    |> result.map_error(fn(_) { "C: no offer 1 id" }),
  )
  use _ <- result.try(
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_c.id,
      dev_user.id,
      "Hmm, Samstag geht auch, aber erst abends. Geht das klar?",
    )
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_c.id,
      dev_user.id,
      "Und kann ich sie vielleicht schon Freitagabend holen?",
    )
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_c.id,
      timon.id,
      "Ja, das geht klar. Ich schick dir ein neues Angebot.",
    )
  ...
  use oc2 <- result.try(
    returned.rows
    |> list.first
    |> result.map_error(fn(_) { "C: no offer 2 id" }),
  )
  use _ <- result.try(
  ...
  use _ <- result.try(
    sql.create_message(conn, req_d.id, lisa.id, "Samstag Nachmittag geht klar")
  ...
  use _ <- result.try(
    sql.create_message(conn, req_d.id, lisa.id, "Passt, bis Samstag!")
  ...
  use _ <- result.try(
    sql.mark_rent_request_read(conn, req_d.id, lisa.id)
    |> result.map_error(fn(_) { "D: mark_read lisa" }),
  )

  // ──────────────────────────────────────────────
  // E: Lisa → Internat (Dev) — OPEN, ~11 msgs, 2 offers → long scroll
  // ──────────────────────────────────────────────
  use returned <- result.try(
    sql.create_rent_request(conn, internat_id, lisa.id)
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_e.id,
      lisa.id,
      "Hey, ich würde gern das Internat für ein Projekt nutzen",
    )
  ...
  use _ <- result.try(
    sql.create_message(conn, req_e.id, lisa.id, "Zwei Wochen vielleicht?")
  ...
  use _ <- result.try(
    sql.create_message(conn, req_e.id, lisa.id, "Anfang Juli, 1. bis 14.")
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_e.id,
      dev_user.id,
      "Hmm, zwei Wochen ist schon lang. Würde eine Woche reichen?",
    )
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_e.id,
      lisa.id,
      "Können wir machen, eine Woche ist auch ok",
    )
  ...
  use oe1 <- result.try(
    returned.rows
    |> list.first
    |> result.map_error(fn(_) { "E: no offer 1 id" }),
  )
  use _ <- result.try(
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_e.id,
      lisa.id,
      "Eine Woche ist etwas knapp. Schaffst du 10 Tage?",
    )
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_e.id,
      dev_user.id,
      "Okay, 10 Tage gehen klar. Ich erstelle ein neues Angebot.",
    )
  ...
  use oe2 <- result.try(
    returned.rows
    |> list.first
    |> result.map_error(fn(_) { "E: no offer 2 id" }),
  )
  use _ <- result.try(
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_e.id,
      lisa.id,
      "Perfekt, 10 Tage sind super! Danke!",
    )
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_e.id,
      dev_user.id,
      "Gerne, viel Erfolg mit dem Projekt!",
    )
    |> result.map_error(fn(_) { "E: msg 11" }),
  )
  use _ <- result.try(
    sql.mark_rent_request_read(conn, req_e.id, lisa.id)
    |> result.map_error(fn(_) { "E: mark_read lisa" }),
  )
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_f.id,
      timon.id,
      "Moin, kann ich Inserat 1 ausleihen?",
    )
  ...
  // ──────────────────────────────────────────────
  use returned <- result.try(
    sql.create_rent_request(conn, inserat2_id, lisa.id)
  ...
  use _ <- result.try(
    sql.create_message(conn, req_g.id, lisa.id, "Diesen Freitag, 13. Juni")
  ...
  use _ <- result.try(
    sql.create_message(conn, req_g.id, lisa.id, "Ja, komme gegen 17 Uhr vorbei")
  ...
  use _ <- result.try(
    sql.create_message(conn, req_g.id, lisa.id, "Perfekt, bis Freitag!")
  ...
  use _ <- result.try(
    sql.create_message(
      ...
      req_g.id,
      lisa.id,
      "Ja, danke! Kann es morgen zurückbringen.",
    )
  ...
  use _ <- result.try(
    sql.mark_rent_request_read(conn, req_g.id, lisa.id)
    |> result.map_error(fn(_) { "G: mark_read lisa" }),
  )

  // ──────────────────────────────────────────────
  // H: Dev → Bohrmaschine (Luca) — OPEN, ~4 msgs, no offer yet
  // ──────────────────────────────────────────────
  use returned <- result.try(
    sql.create_rent_request(conn, bohrmaschine_id, dev_user.id)
    |> result.map_error(fn(_) { "H: create_rent_request" }),
  )
  use req_h <- result.try(
    returned.rows |> list.first |> result.map_error(fn(_) { "H: no id" }),
  )
  use _ <- result.try(
    sql.create_message(
      conn,
      req_h.id,
      dev_user.id,
      "Hey, kann ich die Bohrmaschine fürs Wochenende ausleihen?",
    )
    |> result.map_error(fn(_) { "H: msg 1" }),
  )
  use _ <- result.try(
    sql.create_message(
      conn,
      req_h.id,
      luca.id,
      "Klar, für welches Projekt?",
    )
    |> result.map_error(fn(_) { "H: msg 2" }),
  )
  use _ <- result.try(
    sql.create_message(
      conn,
      req_h.id,
      dev_user.id,
      "Ich will ein Regal im Keller aufbauen, brauche sie nur Samstag.",
    )
    |> result.map_error(fn(_) { "H: msg 3" }),
  )
  use _ <- result.try(
    sql.create_message(
      conn,
      req_h.id,
      luca.id,
      "Klingt gut, hol sie dir einfach ab!",
    )
    |> result.map_error(fn(_) { "H: msg 4" }),
  )
  use _ <- result.try(
    sql.mark_rent_request_read(conn, req_h.id, dev_user.id)
    |> result.map_error(fn(_) { "H: mark_read dev" }),
  )
  use _ <- result.try(
    sql.mark_rent_request_read(conn, req_h.id, luca.id)
    |> result.map_error(fn(_) { "H: mark_read luca" }),
  )
...
fn seed_image(
  ...
  use result <- result.try(
    ...
    |> result.map_error(fn(_) {
      "Failed to insert image DB record for " <> filename
    }),
  )
  ...
  use _ <- result.try(
    result.rows
    |> list.first
    |> result.map_error(fn(_) { "No row returned for " <> filename }),
  )
...
fn create_user_with_profile(
  ...
  use user <- result.try(
    ...
    |> result.map_error(fn(e) {
      "Failed to create user: " <> auth_error_message(e)
    }),
  )
...
// TODO: Does this still clear everything?
fn clear_all(conn: pog.Connection) -> Result(Nil, String) {
  use _ <- try_map(sql.delete_all_items(conn), fn(_) { "Failed to clear items" })
  use _ <- try_map(sql.delete_all_profiles(conn), fn(_) {
  ...
  use _ <- try_map(simplifile.delete_all(images), fn(_) {
    "Failed to clear uploaded images"
  })
  Ok(Nil)
}

fn try_map(result, error_fn, callback) {
  result.try(result |> result.map_error(error_fn), callback)
}
...
fn insert_item(
  ...
  use row <- result.try(
    returned.rows
    |> list.first
    |> result.map_error(fn(_) { "No item id returned" }),
  )
